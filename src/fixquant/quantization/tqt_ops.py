...
def _init_threshold_torch(self, x):
    def _max(x):
    ...
    def _kl_j(x, bitwidth=8):
        ...
        hist = torch.histc(x.abs(), bins=bins, min=mn, max=mx.item())
        pdf = hist / hist.sum()
        ...
        if n + 1 > len(bin_edges) - 1:
            ...
            for i in range(n + 1, len(bin_edges)):
                ...
                p = cdf.clone()
                p[i - 1:] = 1
                x = torch.linspace(torch.tensor(0.0, device=x.device),
                ...
                fp = p[:i]
                p_interp = interp(x, xp, fp)
                x = torch.linspace(torch.tensor(0.0, device=x.device),
                                   torch.tensor(1.0, device=x.device), i, device=x.device)
                xp = torch.linspace(torch.tensor(0.0, device=x.device),
                                    torch.tensor(1.0, device=x.device), n, device=x.device)
                fp = p_interp
                q_interp = interp(x, xp, fp)
                q = p.clone()
                q[:i] = q_interp

                d_tmp = calculate_kl_divergence(cdf[torch.nonzero(cdf)], q[torch.nonzero(cdf)])
                d.append(d_tmp.item())
...
if __name__ == '__main__':
    ...
    class Example:
        ...
        def init_threshold(self, x):
            return _init_threshold_np(self,x)
    ...
    class Example2:
        ...
        def init_threshold(self, x):
            return _init_threshold_torch(self, x)
    ...
    threshold_torch = example2.init_threshold(x2)
