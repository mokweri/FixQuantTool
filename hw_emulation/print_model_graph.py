...
def main():
    ...
    quant_config = Path(args.quant_config) if args.quant_config else (REPO_ROOT / "quantization/utils/quant_config.yaml")
    image_path = Path(args.image) if args.image else (Path(__file__).resolve().parent / "new.JPEG")
    out_txt = Path(args.out_txt) if args.out_txt else (REPO_ROOT / "hw_data_files/model_graph.txt")
    out_json = Path(args.out_json) if args.out_json else (REPO_ROOT / "hw_data_files/model_graph.json")
