...
def main():
    ...
    qparams_path = Path(args.qparams_json) if args.qparams_json else (REPO_ROOT / "hw_data_files/layer1_0_conv2_qparams.json")
    weights_path = Path(args.weights_file) if args.weights_file else (REPO_ROOT / "hw_data_files/weights_64x64x3x3.data")
    bias_path = Path(args.bias_file) if args.bias_file else (REPO_ROOT / "hw_data_files/biases_1x64.data")
    activation_path = Path(args.activation_file) if args.activation_file else (REPO_ROOT / "hw_data_files/test_image_64x64x64.data")
    out_path = Path(args.out_file) if args.out_file else (REPO_ROOT / "hw_data_files/t_ref_output.data")
